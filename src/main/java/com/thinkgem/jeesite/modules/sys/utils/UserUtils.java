	...
	public static List<Menu> getMenuList(){
		...
		if (menuList == null){
			...
			for(Menu menu:menuList){
				if(menu.getParent()!=null&&"1".equals(menu.getParent().getId())){
					List<Menu> childMenuList = menu.getChildList();
					if(childMenuList.size()==0){
						menu.setFirstLevelEmpty(true);
					}else{
						//底层有显示的菜单，则表明有子菜单
						boolean flag=true;
						for(Menu childMenu:childMenuList){
							if(childMenu.getIsShow().equals(Menu.SHOW)){
								flag=false;
								break;
							}
						}
						menu.setFirstLevelEmpty(flag);
					}
